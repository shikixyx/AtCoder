...
# A1 ~ Aiまでの和 O(logN)
def BIT_query(BIT, N, idx):
    res_sum = {}
    while idx > 0:
        res_sum = merge(BIT[idx], res_sum)
        idx -= idx & (-idx)
    return res_sum


# Ai += x O(logN)
def BIT_update(BIT, N, idx, x):
    while idx <= N:
        BIT[idx] = merge(x, BIT[idx])
        idx += idx & (-idx)
    return


def BIT_query_range(BIT, N, l, r):
    right = BIT_query(BIT, N, r)
    left = BIT_query(BIT, N, l-1)
...
def HLD(u, parent, root, TOP, PATH, s, BIT, order,  A):
    stack = [(u, parent, root)]
    N = len(s) - 1
    used = [False] * (len(s))
    idx = 1
...
def query(u, v, BIT, order, TOP, DPT, parents, MEMO, N):
    ret = {}
    while TOP[u] != TOP[v]:
        if depth(TOP[u], DPT, parents) <= depth(TOP[v], DPT, parents):
            ...
            r = order[v]
            if not MEMO[l][r]:
                MEMO[l][r] = BIT_query_range(BIT, N, l, r)
            q = MEMO[l][r]
            ret = merge(ret, q)
            v = parents[TOP[v]]
        else:
...
def solve():
    ...
    HLD(1, -1, 1, TOP, PATH, s, BIT, order, A)
    ...
    ans = []
    for u, v in UV:
        ret = query(u, v, BIT, order, TOP, DPT, parents, MEMO, N)
        ans.append(cnt(ret))
