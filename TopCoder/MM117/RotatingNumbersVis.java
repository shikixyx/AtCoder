        ...
        public void paintComponent(Graphics g) {
            synchronized (tc.worldLock) {
                ...
                if (animMove != null) {
                    ...
                    rt.rotate(animAngle, startX + animMove.c * SZ + SZ * animMove.size / 2.0,
                            startY + animMove.r * SZ + SZ * animMove.size / 2.0);
                }
                ...
                for (int step = 0; step <= (animMove != null ? 1 : 0); step++) {
                    ...
                    for (int r = 0; r < tc.N; r++) {
                        if (startY + r * SZ > clip.getMaxY())
                            break;
                        if (startY + r * SZ + SZ < clip.getMinY())
                            continue;
                        if (step == 1 && (r < animMove.r || r >= animMove.r + animMove.size))
                            continue;
                        for (int c = 0; c < tc.N; c++) {
                            if (startX + c * SZ > clip.getMaxX())
                                break;
                            if (startX + c * SZ + SZ < clip.getMinX())
                                continue;
                            if (step == 1 && (c < animMove.c || c >= animMove.c + animMove.size))
                                continue;
                            ...
                            boolean in = animMove != null && r >= animMove.r && r < animMove.r + animMove.size
                                    && c >= animMove.c && c < animMove.c + animMove.size;
...
public class RotatingNumbersVis {
    ...
    public double runTest() {
        ...
        try {
            ...
            System.err
                    .println("ERROR: Unable to execute your solution using the provided command: " + execCommand + ".");
            return -1;
        }
        ...
        try {
            for (int i = 0; i < numMoves; i++) {
                ...
                if (debug)
                    System.err.println("executing " + smove);
                String[] s = smove.split(" ");
                if (s.length != 4) {
                    System.err.println("ERROR: The move command with index " + i
                            + " does not contain 4 space seperated values. Value is [" + smove + "]");
                    return -1;
                }
                int r = -1;
                int c = -1;
                ...
                if (size < tc.minSize || size > tc.maxSize) {
                    System.err.println("ERROR: size must be between " + tc.minSize + " and " + tc.maxSize + ", but "
                            + size + " was given");
                    return -1;
                }
